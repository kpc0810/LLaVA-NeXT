...
def em_cos_score(
        model, refs, hyps, ori_cands, ori_refs, vids, vid_feat_cache, tokenizer, idf_dict, preprocess, verbose=True,
        batch_size=64, device="cuda:0", return_matched_idx=False
):
    """
    Compute EMScore.
    ...
    for batch_start in iter_range:
        ...
        for i, sen in enumerate(sen_batch):
            ...
            # idf = text_idfs[i, 1:sequence_len+1]
            local_emb, global_emb = local_emb.half(), global_emb.half()
    ...
    if refs:
        ...
        with torch.no_grad():
            for batch_start in iter_range:
                ...
                refs_s_cogr = torch.bmm(hyp_stats_global.unsqueeze(1),
                                        ref_stats_global.unsqueeze(1).transpose(1, 2)).squeeze()
    ...
    if vids:
        ...
        with torch.no_grad():
            for batch_start in iter_range:
                ...
                vid_s_cogr = torch.bmm(ori_hyp_stats_global.unsqueeze(1),
                                       ori_vids_stats_global.unsqueeze(1).transpose(1, 2)).squeeze()
