...
from llava.utils import rank0_breakpoint, rank_print
...
class MiraLazyContrastiveDataset(LazySupervisedDataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        for pair in relation_pair:
            act_words += [find_most_similar_substring(caption, pair[0])]
            obj_words += [find_most_similar_substring(caption, word) for word in pair[1:]]
        ...
        self.exclude_invalid_patch_indices(obj_patch_indices)
        data_dict["obj_patch_indices"] = obj_patch_indices

        return data_dict
    
    def exclude_invalid_patch_indices(self, obj_patch_indices):
        num_obj_words = len(obj_patch_indices)
        unwanted_obj_patch_indices = [[[] for i in range(self.data_args.frames_upbound)] for j in range(num_obj_words)]
        if obj_patch_indices == unwanted_obj_patch_indices:
...
class MiraDataCollatorForContrastiveDataset(DataCollatorForSupervisedDataset):
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        obj_patch_indices, obj_words, obj_phrase_indices, act_words, act_phrase_indices = [], [], [], [], []
        for instance in instances:
            ...
            if isinstance(instance["obj_patch_indices"], list) and isinstance(instance["obj_phrase_indices"], list) and isinstance(instance["act_phrase_indices"], list):
                ...
                raise ValueError("obj_patch_indices, obj_phrase_indices, and act_phrase_indices must be lists")
