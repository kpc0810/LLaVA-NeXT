...
def extract_events(model, inputs, verbose=False, is_pred=False, max_retry=10):
    if is_pred:
        caption = inputs['prediction'].lower()
    else:
        caption = inputs['response'].lower()
...
def process_one_sample(model, inputs, verbose=False):
    
    response, prediction = inputs['response'].lower(), inputs['prediction'].lower()
    result = None
    try:
        if isinstance(inputs.get('events', None), list):
            gt_events = inputs['events']
        else:
            gt_events = extract_events(model, inputs, verbose=verbose, is_pred=False)
        pred_events = extract_events(model, inputs, verbose=verbose, is_pred=True)
        ...
        if 'extra_info' in inputs:
            result['extra_info'] = inputs['extra_info']
    except Exception as e:
        if verbose:
            print(e)
            print(f'invalid GPT response: {result}')
        result = None
        return {'success': False, 'result': result, 'data': inputs}
...
def run_process(args, dataset, local_rank, valid_queue, invalid_queue):
    ...
    with open(TEMP_RESULT_TEMPLATE.format(local_rank), "w") as f:
...
def main(main_args):
    ...
    for rank in range(num_gpus):
        tmp = json.load(open(TEMP_RESULT_TEMPLATE.format(rank), "r"))
        results.extend(tmp['results'])
        invalid_results.extend(tmp['invalid_results'])
        os.remove(TEMP_RESULT_TEMPLATE.format(rank))
