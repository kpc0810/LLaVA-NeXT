...
def run_inference(args):
    ...
    for i in tqdm(range(0, len(data))):
        cnt += 1
        sample = data[i]
        video_id, video_file = sample["idx"], sample["video_file"].split("/")[-1]
        ...
        sample["response"] = sample.pop("description")
        sample["dataset"] = sample.pop("source")
        sample["extra_info"] = {
            "duration": sample.pop("duration"),
            "n_subjects": sample.pop("n_subjects"),
            "n_shots": sample.pop("n_shots"),
            "n_events": sample.pop("n_events"),
        }
