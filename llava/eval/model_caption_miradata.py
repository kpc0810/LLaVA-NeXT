...
def parse_args():
    ...
    parser.add_argument("--video_folder", help="Path to the video files.", required=True)
    parser.add_argument("--output-dir", help="Directory to save the model results JSON.", required=True)
    parser.add_argument("--output-name", help="Name of the file for storing results JSON.", required=True)
...
def merge_temp_files(output_dir, output_name, answers_path):
    ...
    if dist.get_rank() == 0:
        ...
        pattern = re.compile(rf"{re.escape(output_name)}\.temp\.\d+\.json")
...
def run_inference(args):
    ...
    with open(data_path, 'r', newline='') as f:
        data = [json.loads(json.dumps(row)) for row in csv.DictReader(f)]
    ...
    temp_answers_file = os.path.join(args.output_dir, f"{args.output_name}.temp.{local_rank}.json")
    ...
    for i in tqdm(range(0, len(data))):
        cnt += 1
        sample = data[i]
        video_file, video_id, clip_id = sample['file_path'], sample['video_id'], sample['clip_id']
        if f"{video_id}-{clip_id}" in cache_set:
        ...
        for round in rounds:
            question = random.choice(user_prompts[round])
            ...
            # save for writing to file
            pred_captions[round] = outputs
            gt_captions[round] = sample[f"{round}_caption"]
            sample.pop(f"{round}_caption")
        
        # save prediction and gt captions, while pop the unneeded elements
        sample["pred_captions"] = pred_captions
        sample["gt_captions"] = gt_captions
        sample.pop("short_caption"); sample.pop("style_caption"); sample.pop("camera_caption")
        sample.pop("dense_relation_pair"); sample.pop("main_object_relation_pair"); sample.pop("background_relation_pair")
