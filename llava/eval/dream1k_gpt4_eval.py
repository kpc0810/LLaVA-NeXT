...
class DREAMGPTMetric:
    ...
    def _process(self, dataset: List[Dict], subtask=None):
        ...
        results = pool.map(process_one_sample, inputs)
        breakpoint()
...
def argmument():
    ...
    parser.add_argument('--pred_path', type=str, default='outputs/dream1k')
...
if __name__ == '__main__':
    ...
    dataset = [{
                "idx": 1, 
                "dataset": "DREAM/movie_animation",
                "video_file": "video/1.mp4", 
                "source": "DREAM/movie_animation", 
                ...
                "extra_info": {"duration": 5.09, "n_subjects": 4, "n_shots": 2, "n_events": 5},
                "prompt": "<image>\nThe video lasts for 5.09 seconds, and 64 frames are uniformly sampled from it. These frames are located at 0.00s,0.04s,0.13s,0.21s,0.29s,0.38s,0.46s,0.54s,0.63s,0.71s,0.79s,0.88s,0.96s,1.00s,1.08s,1.17s,1.25s,1.33s,1.42s,1.50s,1.58s,1.67s,1.75s,1.83s,1.92s,2.00s,2.04s,2.13s,2.21s,2.29s,2.38s,2.46s,2.54s,2.63s,2.71s,2.79s,2.88s,2.96s,3.00s,3.09s,3.17s,3.25s,3.34s,3.42s,3.50s,3.59s,3.67s,3.75s,3.84s,3.92s,4.00s,4.05s,4.13s,4.21s,4.30s,4.38s,4.46s,4.55s,4.63s,4.71s,4.80s,4.88s,4.96s,5.05s.Please answer the following questions related to this video.\nPlease provide a detailed description of the video, focusing on the main subjects, their actions, and the background scenes.", 
                "response": "A character emerges from smoke and fire, then half knelt on the ground and made a gesture to give instructions, while another group of characters rushed forward and grabbed two green monsters.",
                "prediction": "The video opens with a close-up of a large, orange, humanoid robot with multiple eyes and limbs. The robot is in a dimly lit environment filled with smoke and debris, suggesting a chaotic or post-apocalyptic setting. The robot's body is bulky, with a rounded torso and limbs that end in what appear to be mechanical hands. It has a number \"0414\" printed on its chest. As the video progresses, more robots of similar design are revealed, each with their own unique numbers and features. They seem to be moving towards the camera, creating a sense of urgency or threat. In the background, there are glimpses of green, furry creatures that resemble monsters from popular culture, adding to the fantastical element of the scene. The lighting is low, casting shadows and giving the environment an eerie atmosphere.", "gt_captions": "A character emerges from smoke and fire, then half knelt on the ground and made a gesture to give instructions, while another group of characters rushed forward and grabbed two green monsters."
    }]
    metric = DREAMGPTMetric(dataset_name, verbose=True)
    metric.process(dataset)
    metric.summarize_metric()
    metric.save_results(pred_path)
    metric.save_eval_infos(pred_path)